...
By default, your app stores data **locally on the device** using SwiftData. This is the simplest option and requires no setup.

| | Local Storage (Default) | Cloud Storage (Supabase) |
|--|------------------------|-------------------------|
| **Setup** | None | Create Supabase account |
| **User accounts** | No | Yes |
| **Sync across devices** | No | Yes |
| **Share between users** | No | Yes |
| **Data persists if app deleted** | No | Yes |
| **Cost** | Free | Free tier available |

### When to Use Local Storage

- Simple utilities (calculator, timer, notes)
- Personal apps without accounts
- Prototyping and testing
- Apps that work offline

### When to Use Supabase

- Apps with user signup/login
- Data that syncs across devices
- Social features (sharing, following)
- Data that must persist even if app is deleted

<Tip>
  Start with local storage. If you need cloud features later, Nativeline makes it easy to add Supabase.
</Tip>
...
<Steps>
  <Step title="Open Project Settings">
    Go to the **Project Settings** tab → **Database** panel in your Nativeline project
  </Step>
...
The Database tab in Nativeline lets you view and manage your Supabase data directly.

### Navigating the Database Tab

When you open the Database tab, you'll see:

1. **Table List** - All your database tables on the left
2. **Data View** - The selected table's contents
3. **View Toggle** - Switch between Data and Schema views

### Data View

The Data View shows your actual data in a spreadsheet-like grid.

**Toolbar:**
- **Search bar** - Filter rows by searching text
...
- Null values appear grayed out
- Boolean values show as colored indicators

**Row Inspector:**
When you click a row, the Row Inspector panel appears on the right showing:
- All column values for that row
- Easier viewing of long text fields
- Related data (if foreign keys exist)

**Pagination:**
- Navigate between pages for large tables
- Adjust rows per page

### Schema View

Switch to Schema View to see how your table is structured.
...
View pending migrations and apply them with one click.
